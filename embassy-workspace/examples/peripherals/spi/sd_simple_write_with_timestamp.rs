...
static TZ: jiff::tz::TimeZone = jiff::tz::get!("Europe/Moscow");
...
async fn main(spawner: Spawner) -> ! {
    ...
    // CURRENT_TIME_US="$(printf '%.0f\n' "$(echo "$EPOCHREALTIME * 1000000" | bc)")" -> текущее время в микросекундах

    if let Ok(current_time_us) = env!("CURRENT_TIME_US").parse() {
        rtc.set_current_time_us(current_time_us);
    } else {
        error!("Invalid microseconds, should be in the format: YYYY-MM-DDT HH:MM:SS");
    }
    ...
    let volume_mgr = embedded_sdmmc::VolumeManager::new(sdcard, sd_timer);
