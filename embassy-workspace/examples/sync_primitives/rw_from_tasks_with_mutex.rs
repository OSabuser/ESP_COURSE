...
static SHARED_CNT: Mutex<CriticalSectionRawMutex, RefCell<u32>> = Mutex::new(RefCell::new(0));
...
async fn main(spawner: Spawner) -> ! {
    ...
    loop {
        ...
        let shared = SHARED_CNT.lock(|f| f.clone().into_inner());
...
async fn simple_task() {
    loop {
        ...
        SHARED_CNT.lock(|f| {
            let temp_val = f.borrow_mut().wrapping_add(4);
            f.replace(temp_val);
        });
...
async fn simple_task_second() {
    loop {
        SHARED_CNT.lock(|f| {
            let temp_val = f.borrow_mut().wrapping_sub(3);
            f.replace(temp_val);
        });
